...
```
  ...
  |  | tdi_docs_{slug}   × 10 (bim, mep, marketing, phap_ly,   | |
  |  | tdi_videos_{slug} × 10   san_xuat, cong_nghe, nhan_su,  | |
  |  |                         tai_chinh, kinh_doanh, thiet_ke)| |
  |  | Ingestion route theo metadata.domain                    | |
...
[STORE] Qdrant — route theo metadata.domain:
    collection = tdi_docs_{slug}  (VD: tdi_docs_bim, tdi_docs_cong_nghe)
    domain BẮT BUỘC có ở form upload; _resolve_domain_store raise ValueError
      nếu trống hoặc không ∈ 10 persona key hợp lệ
...
Domain là `Literal` enum trong Pydantic → LLM **không thể sinh label ngoài 10 giá trị** (constrained decoding ở API level). Upload bắt buộc chọn 1 trong 10 domain → không còn nhãn `"mặc định"` ở classifier (persona `"mặc định"` chỉ còn dùng cho chat chung). `"kết cấu"` đã được gộp vào `"thiết kế"` để khớp với UI 10 domain. `DOMAIN_VALUES` được derive từ `get_args(DomainLiteral)`, kèm assert đầu module: (a) `DOMAIN_VALUES ⊂ DOMAIN_PERSONAS.keys()`, (b) `DOMAIN_PERSONAS.keys() − DOMAIN_VALUES == {"mặc định"}` — build fail ngay nếu ai đổi persona mà quên sync classifier. Tags được normalize sau (lowercase, dedup, strip punctuation, cap 8).
...
### Domain chuyên gia (10 persona + 1 chat-chung)

| Domain | Vai trò | Thuật ngữ chốt (không dịch) | Upload | Chat |
|--------|---------|-----------------------------|:------:|:----:|
| `mặc định` | Trợ lý Tri thức TDI tổng quát | — | ❌ | ✅ (chat chung) |
...
| `thiết kế` | Giám đốc Thiết kế 12+ năm (gồm cả kết cấu) | concept vs schematic design, shop drawing, BTCT, mác bê tông | ✅ | ✅ |

Mỗi persona có khối **PHẠM VI** (trả lời / không trả lời) và **THUẬT NGỮ CHUẨN** (tránh dịch sai gây nhiễu).

- **Upload**: bắt buộc chọn 1 trong 10 domain concrete. Form gửi `domain=""` hoặc `"mặc định"` → `_validate_domain` reject với HTTP 200 error message.
- **Chat**: FE gửi persona key (VD `"công nghệ thông tin"`). `chat.py` map `"mặc định"`/`"general"` → `None` → retriever vào nhánh fanout 20 collection. Domain khác → route single collection pair.

Lookup case-insensitive + trim (`.lower().strip()`) — FE có thể truyền `"BIM"` hoặc `"  bim  "` đều map đúng. Registry `get_by_persona` nhận cả persona key (`"công nghệ thông tin"`) lẫn slug (`"cong_nghe"`) cho linh hoạt.
