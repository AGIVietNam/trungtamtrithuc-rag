...
3. **Async ingest + batch upload** — endpoint `*/async` trả `job_id` ngay, worker pool xử lý nền; endpoint `/files` upload nhiều file 1 request; `/from-urls` nhận list URL có Bearer token (cho BE đẩy file từ S3/SharePoint sau khi duyệt). Polling tiến độ qua `GET /jobs/{id}` và `/batches/{id}`. Streaming upload 1MB/chunk → file 5GB không OOM.
4. **Public URL ingest** — endpoint `/from-url` + `/from-url/preview` nhận link bất kỳ, AI tự resolve theo nguồn (Google Drive/Docs/Sheets/Slides, OneDrive, Dropbox, SharePoint anonymous, YouTube, Vimeo/TikTok/FB/IG/Twitter/Twitch — yt-dlp ~1500 site, generic HTTP). Reject với message tiếng Việt cho folder share / Forms / SharePoint private (cần BE auth). Friendly error 401/403/404/HTML.
...
| `POST /api/ingest/from-url` | 1 URL public bất kỳ — AI auto-detect nguồn | FE tab "Liên kết" sau khi user paste URL+duyệt |
| `POST /api/ingest/from-url/preview` | Preview metadata từ URL (gen title/desc/domain/tags) | FE bấm "Đọc liên kết" để prefill form |
...
| `DELETE /api/ingest/document/{document_id}?domain={slug}` | Xoá toàn bộ chunks Qdrant theo BE document.id (idempotent) | BE xoá S3 + DB record xong → call AI dọn vector |
...
# property message → câu thông báo VN sẵn cho FE hiện toast:
#   done   → "Đã lưu xong '{filename}' vào kho tri thức ({chunks_added} chunks, {pages} trang)"
#   failed → "Lưu '{filename}' thất bại: {error}"
#   khác   → "" (FE không hiện gì khi job chưa terminal)
```
...
- `to_dict()` luôn include field `message` → BE poll `/jobs/{id}` chỉ cần forward field này cho FE hiện toast, không cần ghép câu thủ công.
...
### Public URL handling — `/from-url` + `/from-url/preview`

Tab "Liên kết" của FE: user paste 1 URL bất kỳ → AI resolve nguồn → tải → ingest. Endpoint dùng:

- `POST /api/ingest/from-url` — submit thật, trả `job_id`, worker xử lý nền
- `POST /api/ingest/from-url/preview` — gen metadata (title/description/domain/tags) cho FE prefill form. Tải đoạn đầu (5 trang doc / 3 phút video) → Haiku gen.
- `POST /api/ingest/from-urls` — batch list URL (BE bắn sau khi admin duyệt nhiều file)

**Module resolver `app/core/url_resolver.py`** detect nguồn theo URL pattern:

| Nguồn | Pattern | Convert → |
|---|---|---|
| **Google Drive** file | `drive.google.com/file/d/{id}`, `/file/u/N/d/`, `?id=`, `uc?id=` | `uc?export=download&id={id}&confirm=t` |
| **Google Docs** | `docs.google.com/document/d/{id}` | `/export?format=docx` |
| **Google Sheets** | `docs.google.com/spreadsheets/d/{id}` | `/export?format=xlsx` |
| **Google Slides** | `docs.google.com/presentation/d/{id}` | `/export?format=pptx` |
| **Google Drawings** | `docs.google.com/drawings/d/{id}` | `/export?format=pdf` |
| **OneDrive personal** | `1drv.ms`, `onedrive.live.com` | append `?download=1` |
| **Dropbox** | `dropbox.com/s/`, `/scl/fi/`, `dropboxusercontent.com` | `dl=0` → `dl=1` |
| **SharePoint anonymous** | `:b:/g/`, `:w:/g/`, `:x:/g/`, `:p:/g/` | append `download=1` |
| **YouTube** | `youtube.com`, `youtu.be`, `music.youtube.com` | pass-through (yt-dlp + transcript) |
| **Stream platforms** | Vimeo, TikTok, Facebook, Instagram, Twitter/X, Twitch, Dailymotion, Bilibili | pass-through (yt-dlp ~1500 site) |
| **Generic HTTP** | mọi `http(s)://...` | stream-download trực tiếp |

**Reject với message tiếng Việt rõ** (ValueError):

- Google Drive folder (`/drive/folders/`) — cần Drive API liệt kê items, phase này không support
- Google Forms (`docs.google.com/forms/`) — không export tài liệu được
- Dropbox folder (`/sh/`) — share file đơn lẻ
- SharePoint folder (`:f:/g/`)
- SharePoint **private** (mọi link không phải `:?:/g/` anonymous) — cần Microsoft Graph + token user, BE/FE phải resolve trước rồi đẩy `/from-urls` với headers Bearer
- URL không phải HTTP/HTTPS

**Edge cases handle trong `JobRunner._resolve_file`**:

- HTTP 401/403/404/429/5xx → message tiếng Việt rõ thay vì raw stacktrace
- `Content-Type: text/html` (login page / error page) → reject sớm với message rõ
- File 0 byte sau download → reject
- `Content-Disposition: filename="..."` → override `suggested_filename` (server biết tên chuẩn)
- Multi-account `/u/N/` Google → regex bắt được
- URL `#anchor` → strip
- HTTP redirect → `httpx` follow

### Xoá tài liệu — `DELETE /api/ingest/document/{document_id}`

AI không sở hữu S3 — BE giữ S3 + DB. Khi user xoá ở FE, BE **xoá S3 object + DB record trước**, rồi call AI để dọn vector trong Qdrant. Tách trách nhiệm rõ: AI chỉ lo phần Qdrant.

**Flow:**

```
FE click xoá
  └→ BE   DELETE /api/v1/document/{id}
       1. BE.s3Service.deleteFile(fileKey)
       2. BE call AI: DELETE /api/ingest/document/{document_id}?domain={slug}
       3. BE.documentRepository.delete(id)
       → 200 cho FE
```

**Cơ chế filter:**

- Pipeline ingest (`doc_pipeline.ingest_document`, `video_pipeline.ingest_video_file/ingest_youtube/ingest_youtube_playlist`) nhận `document_id` từ `Job.document_id` (BE pre-allocate UUID + truyền qua `/from-url` body) → lưu thành `payload.document_id` trên mỗi point Qdrant.
- `QdrantRegistry.ensure_all` index `document_id` ở mọi collection (filter delete chạy O(log N) thay vì O(N)).
- Endpoint dùng `count_by_filter` đếm trước → `delete_by_filter` xoá. Nếu BE truyền `?domain=bim` → khoanh 2 collection (`tdi_docs_bim` + `tdi_videos_bim`); không truyền → quét cả 20 collection (chậm hơn nhưng vẫn idempotent).

**Response:**

```json
{
  "document_id": "uuid-xyz",
  "deleted_chunks": 42,
  "collections_affected": ["tdi_docs_bim"]
}
```

**Idempotent**: doc đã không còn chunks → vẫn 200, `deleted_chunks=0`. BE retry an toàn.

**Hạn chế biết trước**: chunks ingest **trước khi** thay đổi này merge không có `document_id` trong payload → không xoá được qua API mới. Cần re-ingest hoặc dùng `delete_by_filter` thủ công theo `doc_id` (content hash).
...
│   │                            # URL: /from-url, /from-url/preview (auto-detect nguồn)
...
│   │   │                        #   friendly error 4xx/5xx/HTML, Content-Disposition
│   │   └── url_resolver.py      # Detect URL pattern → download_url + kind
│   │                            #   GDrive/GDocs/OneDrive/Dropbox/SharePoint/
│   │                            #   YouTube/Vimeo/TikTok/FB/IG/Twitter/Twitch/...
│   │                            #   Reject folder/Forms/SharePoint private friendly
│   ├── ingestion/
...
### Đã hoàn thành (phase async ingest + URL handling)

- ~~**Streaming response chat**~~ ✅ `POST /api/chat/stream` SSE.
- ~~**Prompt cache 2 breakpoint**~~ ✅ system stable + history assistant.
- ~~**GPU reranker**~~ ✅ Auto cuda > mps > cpu + warmup.
- ~~**S3 domain-slug layout**~~ ✅ `docs/<slug>/<sha>.<ext>`.
- ~~**Qdrant 20 collection per-domain**~~ ✅ `tdi_{docs,videos}_{slug}`.
- ~~**Slug alignment NestJS**~~ ✅ ASCII slug khớp `categories.seeder.ts`.
- ~~**Async ingest + worker pool**~~ ✅ `JobRunner` asyncio.Queue, 2 workers, fault isolation.
- ~~**Streaming upload (bigsize)**~~ ✅ 1MB/chunk, cap `MAX_UPLOAD_MB=5GB`, sha256 in-flight.
- ~~**Batch upload**~~ ✅ `/files` (multipart array) + `/from-urls` (JSON list URL).
- ~~**Preview "đọc đoạn đầu"**~~ ✅ `parse(max_pages=5)` + `transcribe(clip_duration_sec=180)`. File 422 trang preview ~13s thay vì ~10 phút.
- ~~**Public URL ingest đa nguồn**~~ ✅ `app/core/url_resolver.py`: GDrive/GDocs/Sheets/Slides/Drawings + multi-account, OneDrive, Dropbox, SharePoint anonymous, YouTube + Vimeo/TikTok/FB/IG/Twitter/Twitch (yt-dlp ~1500 site). Endpoint `/from-url` + `/from-url/preview` + `/from-urls` batch.
- ~~**Friendly download errors**~~ ✅ Catch HTTPStatusError → message tiếng Việt theo status (401/403/404/429/5xx). Detect `Content-Type: text/html` (login page). Validate file 0 byte. Override filename theo `Content-Disposition`.
- ~~**Reject smart**~~ ✅ Folder Drive/Dropbox/SharePoint, Google Forms, SharePoint private — message tiếng Việt rõ thay vì stacktrace.
- ~~**Bug fix pptx preview**~~ ✅ `_DOC_SUFFIXES` thêm `.pptx`, `.ppt`.

### Còn lại (phase sau)

- **BE NestJS — endpoint duyệt + call AI**: BE chưa có `PATCH /document/:id/approve` + `AiClient` service. Khi BE code 4 việc (approve endpoint, AI_URL config, HttpService wrapper, hook trigger sau approve) → flow upload → duyệt → ingest tự động hoạt động. AI side đã sẵn sàng nhận.
- **BE — endpoint URL-only submit**: Tab "Liên kết" trong FE cần BE `POST /document/from-url` (lưu URL không upload S3). Hiện DocumentEntity có field `url` nhưng controller chỉ accept multipart file.
- **BE — mime whitelist mở rộng**: BE thiếu `.md`, `.txt`. Tăng `MAX_FILE_SIZE` từ 200MB lên >2GB nếu muốn dùng được streaming upload AI.
- **SharePoint TDI nội bộ (private)**: Khi BE/FE Microsoft auth resolve link qua Graph → đẩy `/from-urls` với `headers: {Authorization: Bearer ...}`. AI đã sẵn sàng nhận, không cần code thêm.
- **PPTX slide notes**: Hiện `python-pptx` extract text shape nhưng bỏ slide notes (RAG gold). Thêm `slide.notes_slide.notes_text_frame.text` ~5 dòng code.
- ~~**`DELETE /document/{id}`**~~ ✅ Endpoint `DELETE /api/ingest/document/{document_id}` xoá chunks Qdrant theo `payload.document_id`. Pipeline (`doc_pipeline`/`video_pipeline`) lưu `document_id` vào payload mỗi chunk; `QdrantRegistry.ensure_all` index sẵn field này. Idempotent. BE chỉ cần xoá S3 + DB rồi call. Tài liệu cũ ingest trước thay đổi này không có `document_id` trong payload → không xoá được qua API mới (chấp nhận).
- **Persistent job store**: In-memory mất khi restart. Sau khi BE setup RabbitMQ → swap `InMemoryJobStore` → `RedisJobStore` cùng interface. Pipeline + handler không phải sửa.
- ~~**Status callback**~~ ✅ AI publish webhook tới `callback_url` (truyền trong `FromUrlRequest`/`FromUrlsItem`) khi job done/failed. Best-effort, không retry — fallback BE poll `/jobs/{id}`. Response `/jobs/{id}` còn kèm field `message` (câu thông báo VN soạn sẵn) → BE chỉ forward thẳng cho FE hiện toast.
- **Auth thật**: `user_id` chat hiện là string client truyền tự do. Tích hợp Microsoft auth từ BE vào chat layer.
- **Admin CRUD knowledge**: delete/re-index từ FE knowledge page.
- **Evaluation harness**: tập câu hỏi + ground truth đo recall/precision.
